...
use rustc_hir::{Expr, ExprKind, HirId, Node, QPath, TyKind};
...
pub(crate) fn check<'tcx>(cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>, io_functions: &Vec<String>) {
            ...
            if let TyKind::Path(QPath::Resolved(None, p)) = &ty.kind {
                if let Res::Def(DefKind::Struct, def_id) = p.res {
                    if cx.tcx.crate_name(def_id.krate).as_str() == "libloading"
                        && p.segments.last().unwrap().ident.as_str() == "Library"
                    {
                        loading = Some(expr.span);
                        if segment.ident.name == rustc_span::sym::new && !params.is_empty() {
    ...
    let Some(libname) = libname else { return };
